...
class SCR_BF_ThermalPIPSightsComponentClass : SCR_2DPIPSightsComponentClass
{
}

//------------------------------------------------------------------------------------------------
class SCR_BF_ThermalPIPSightsComponent : SCR_2DPIPSightsComponent
{
	[Attribute("{092A79DA0E22563F}UI/Materials/Editor/BF_Thermal_Base.emat", UIWidgets.EditBox, "Thermal effect resource\n", params: "emat", category: "PiPSights")]
	...
	override void SetPIPEnabled(bool enabled)
	{
		...
		if (enabled && !m_bPIPIsEnabled)
		{
			...
			if (m_pMaterial)
				...
				GetGame().GetWorld().SetCameraPostProcessEffect(m_iCameraIndex, 11, PostProcessEffectType.ThermalImaging, m_rScopeThermalMaterial);
	...
	//------------------------------------------------------------------------------------------------
	protected override void DestroyCamera(CameraBase camera)
	{
		if (camera)
		{
			...
			camera.GetWorld().SetCameraPostProcessEffect(m_iCameraIndex, 11, PostProcessEffectType.ThermalImaging, string.Empty);
